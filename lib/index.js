...
class ElementalApi {
  ...
  request(method, path, params, body, options) {
    const url = `${this.secure ? 'https' : 'http'}://${this.host}${path}`;
    const queryString = querystring.stringify(params);
    const urlStr = queryString ? [url, queryString].join('?') : url;
    ...
    return this.parallelStream.add(method, urlStr, params, body, options);
  }
}
