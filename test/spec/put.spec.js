...
test('put', async t => {
  ...
  const result = await api.put(path, params, body, options);
  t.is(typeof result.abc, 'number');
  t.is(result.abc, 123);
  t.is(spyFetch.callCount, 1);
  t.is(spyFetch.getCall(0).args[0], `http://${host}${path}?abc=def&ghi=123`);
  t.is(spyFetch.getCall(0).args[1].method, 'PUT');
  t.is(spyFetch.getCall(0).args[1].headers['Content-Type'], 'application/json');
});
